...
import { removeFromHistory, removeAllHistory } from "../app/userSlice";

const cx = classNames.bind(styles);

const History = () => {
    const history = useSelector((state: any) => state.userStore.history);
    ...
    useEffect(() => {
        dispatch(setGenre(route.history));
    }, []);
    ...
    const handleRemove = async (id: number) => {
        const res: any = await userApi.removeFromHistory(id);
        if (!res?.success || res.code === "ERR_NETWORK") {
            showToast();
            return;
        }

        dispatch(removeFromHistory({ movieID: id }));
        return;
    };

    const handleRemoveAll = async () => {
        const res: any = await userApi.removeAllHistory();
        if (!res?.success || res.code === "ERR_NETWORK") {
            showToast();
            return;
        }
        ...
        <div className={cx("main")}>
            <FilmContainer ref={filmContainerRef} className={"content"}>
                {isAuth ? (
                    history &&
                    (history.length > 0 ? (
                        history.map((film: any, index: number) => {
                            return (
            ...
            {isAuth && history && history.length > 0 && (
                <div
